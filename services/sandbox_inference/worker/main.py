from __future__ import annotations

import asyncio
import logging
import os
import sys
from pathlib import Path

_REPO = Path(__file__).resolve().parents[3]
if str(_REPO) not in sys.path:
    sys.path.insert(0, str(_REPO))

from shared.logging_config import setup_logging

setup_logging("sandbox_inference")

from shared.messaging.redis_streams import JobBroker, StreamsConfig, work_stream
from shared.protocol.envelope import ServiceName
from services.sandbox_inference.worker.processor import process_sandbox

logger = logging.getLogger("sandbox_inference")


async def run() -> None:
    svc = ServiceName.sandbox_inference
    broker = JobBroker(
        StreamsConfig(
            redis_url=os.environ.get("REDIS_URL", "redis://localhost:6379/0"),
            consumer_name=os.environ.get("WORKER_CONSUMER", "sandbox-1"),
        )
    )
    await broker.connect()
    await broker.ensure_groups([svc])
    stream = work_stream(svc)
    while True:
        batch = await broker.read_work(svc, count=1, block_ms=8000)
        for entry_id, env in batch:
            try:
                await process_sandbox(env)
            except Exception:
                logger.exception("sandbox failed")
            finally:
                await broker.ack(stream, entry_id)


if __name__ == "__main__":
    asyncio.run(run())
