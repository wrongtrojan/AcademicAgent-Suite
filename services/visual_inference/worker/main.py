...
setup_logging("visual_inference")
...
from services.visual_inference.worker.processor import process_visual

logger = logging.getLogger("visual_inference")


async def run() -> None:
    svc = ServiceName.visual_inference
    broker = JobBroker(
        StreamsConfig(
            redis_url=os.environ.get("REDIS_URL", "redis://localhost:6379/0"),
            consumer_name=os.environ.get("WORKER_CONSUMER", "visual-1"),
        )
    )
    ...
    while True:
        ...
        for entry_id, env in batch:
            try:
                await process_visual(env)
            except Exception:
                logger.exception("visual failed")
            finally:
