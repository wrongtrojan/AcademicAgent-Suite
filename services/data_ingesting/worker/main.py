...
from services.data_ingesting.worker.processor import process_ingest

logger = logging.getLogger("data_ingesting")


async def run() -> None:
    svc = ServiceName.data_ingesting
    broker = JobBroker(
        StreamsConfig(
            redis_url=os.environ.get("REDIS_URL", "redis://localhost:6379/0"),
            consumer_name=os.environ.get("WORKER_CONSUMER", "ingest-1"),
        )
    )
