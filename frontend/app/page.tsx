...
export default function ScaffoldingPage() {
  ...
  const normalizeAsset = useCallback((backendData: any): Asset => {
    ...
    const aid = backendData.asset_id as string;
    const t = typeof backendData.title === 'string' ? backendData.title.trim() : '';
    return {
      id: aid,
      name: aid,
      title: t || undefined,
      type: backendData.asset_type || (aid.endsWith('.pdf') ? 'pdf' : 'video'),
      status: status,
